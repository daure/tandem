...
class ReleaseTests(unittest.TestCase):
    ...
    def test_stable_version_bumps(self):
        for bump, expected in [("patch", "0.1.1"), ("minor", "0.2.0"), ("major", "1.0.0")]:
            self.assertEqual(release.next_version("0.1.0", bump), expected)
        with self.assertRaises(ValueError):
            release.next_version("0.1.0-rc.1", "patch")
        ...
        with tempfile.TemporaryDirectory() as directory, patch.dict(os.environ, environment):
            ...
            try:
                with patch.object(release, "__file__", str(repo / "scripts/release.py")), patch.object(release, "run", side_effect=run):
                    release.release("patch")
                self.assertIn('version = "0.1.1"', (repo / "Cargo.toml").read_text())
                self.assertEqual(git("rev-parse", "HEAD"), git("rev-parse", "v0.1.1^{commit}"))
                self.assertIn(git("rev-parse", "HEAD"), git("ls-remote", "origin", "refs/heads/main"))
                self.assertIn("refs/tags/v0.1.1", git("ls-remote", "origin", "refs/tags/v0.1.1"))
                self.assertIn(
                    (("git", "push", "--atomic", "origin", "HEAD:refs/heads/main", "refs/tags/v0.1.1"), {}),
                    calls,
                )
                tag_call = next(
                    call
                    for call in calls
                    if call[0] == ("git", "tag", "-a", "v0.1.1", "-m", "release: v0.1.1")
                )
                ...
                self.assertEqual(next(p["version"] for p in packages if p["name"] == "tandem"), "0.1.1")
                self.assertEqual(git("status", "--porcelain"), "")
            finally:
